...
import { requireCond, tostr } from '../utils/utils'
...
export class ReputationManager {
    ...
    getStatus(addr?: string): ReputationStatus {
        ...
        const minExpectedIncluded = Math.floor(
            entry.opsSeen / this.params.minInclusionDenominator
        )
        if (
            minExpectedIncluded <=
    ...
    checkStake(
        ...
        requireCond(
            ...
            `${title} ${info.addr} stake ${tostr(
                info.stake
            )} is too low (min=${tostr(this.minStake)})`,
        ...
        requireCond(
            ...
            )} is too low (min=${tostr(this.minUnstakeDelay)})`,
            ValidationErrors.InsufficientStake
        )
    }
}
