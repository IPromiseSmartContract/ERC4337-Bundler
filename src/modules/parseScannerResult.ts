...
import { toBytes32 } from './moduleUtils'
...
export function parseScannerResult(
    ...
    if (Object.values(tracerResults.numberLevels).length < 2) {
        ...
        throw new Error(
            'Unexpected traceCall result: no NUMBER opcodes, and not REVERT'
        )
    }
    ...
    Object.entries(stakeInfoEntities).forEach(
        ([entityTitle, entStakes], index) => {
            ...
            Object.entries(access).forEach(([addr, { reads, writes }]) => {
                ...
                ;[...Object.keys(writes), ...Object.keys(reads)].forEach(
                    (slot) => {
                        ...
                        if (associatedWith(slot, sender, entitySlots)) {
                            ...
                            associatedWith(slot, entityAddr, entitySlots)
                        ) {
            ...
            function requireCondAndStake(
                ...
                requireCond(
                    BigNumber.from(1).lt(entStakes.stake) &&
                        BigNumber.from(1).lt(entStakes.unstakeDelaySec),
                    failureMessage,
                    ValidationErrors.OpcodeValidation,
                    { [entityTitle]: entStakes?.addr }
                )
            ...
            requireCond(
                Object.keys(currentNumLevel.contractSize).find(
                    (addr) => currentNumLevel.contractSize[addr] <= 2
                ) == null,
    ...
    const addresses = tracerResults.numberLevels.flatMap((level) =>
