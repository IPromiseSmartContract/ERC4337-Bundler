...
export class BundleManager {
    ...
    async sendNextBundle(): Promise<SendBundleReturn | undefined> {
        return await this.mutex.runExclusive(async () => {
            ...
            if (bundle.length === 0) {
                ...
                const ret = await this.sendBundle(
                    bundle,
                    beneficiary,
                    storageMap
                )
                debug(
                    `sendNextBundle exit - after sent a bundle of ${bundle.length} `
                )
                return ret
            }
        })
    ...
    async sendBundle(
        ...
        try {
            ...
            if (this.conditionalRpc) {
                ...
                ret = await this.provider.send('eth_sendRawTransaction', [
                    signedTx,
                ])
            ...
            try {
                parsedError = this.entryPoint.interface.parseError(
                    e.data?.data ?? e.data
                )
            } catch (e1) {
                this.checkFatal(e)
            ...
            if (reasonStr.startsWith('AA3')) {
                this.reputationManager.crashedHandleOps(
    ...
    async createBundle(): Promise<[UserOperation[], StorageMap]> {
        ...
        for (const entry of entries) {
            ...
            if (paymaster != null) {
                if (paymasterDeposit[paymaster] == null) {
                    paymasterDeposit[paymaster] =
                        await this.entryPoint.balanceOf(paymaster)
                }
                if (
                    paymasterDeposit[paymaster].lt(
            ...
            if (
                this.mergeToAccountRootHash &&
                this.conditionalRpc &&
                entry.userOp.initCode.length <= 2
            ) {
                const { storageHash } = await this.provider.send(
                    'eth_getProof',
